...
class RTSPEnv:
    ...
    def step(self, node_idx, CUDA_DEVICE_NUM):
        ...
        if done:
            reward = -self.get_maxregret_travel_distance(CUDA_DEVICE_NUM)  # Note the MINUS Sign ==> We MAXIMIZE reward
            # reward = -self.get_maxregret_travel_distance_lkh()
            # reward = -self.get_maxregret_travel_distance_cmes()
            # shape: (batch, pomo)
        else:
            reward = None
        return self.step_state, reward, done
    ...
    def get_maxregret_travel_distance_lkh(self):
        dis_up = self.dis_up.unsqueeze(1).repeat(1, self.pomo_size, 1, 1)
        dis_down = self.dis_down.unsqueeze(1).repeat(1, self.pomo_size, 1, 1)
        ...
        # cost_y
        # (batch,pomo,problem+1,problem+1)
        node_cnt = final_dis.size(-1)

        wcr_matrix = [[0] * node_cnt for i in range(node_cnt)]
        route_y1 = torch.zeros(self.batch_size, self.pomo_size, node_cnt)
        for i in range(self.batch_size):
            for j in range(self.pomo_size):
                wcr_matrix = final_dis[i][j].cpu().numpy()
                temp_list_y = elkai.solve_float_matrix(wcr_matrix)
                route_y1[i][j] = torch.tensor(temp_list_y)
                del wcr_matrix

        y = route_y1.clone().type(torch.int64)
        link_y = self.get_link_seq(y)
        link_y = torch.unsqueeze(link_y, dim=3)
        cost_y1 = final_dis.gather(-1, link_y).sum(2).squeeze(-1)

        max_regret = cost_x - cost_y1

        return max_regret

    def get_maxregret_travel_distance_cmes(self):
        dis_up = self.dis_up.unsqueeze(1).repeat(1, self.pomo_size, 1, 1)
        dis_down = self.dis_down.unsqueeze(1).repeat(1, self.pomo_size, 1, 1)
        ...
        samples = 256
        ites = 100
        route_y, cost_y = FastCMA_ES(final_dis, 0.5, samples, ites)

        max_regret = (cost_x - cost_y)
