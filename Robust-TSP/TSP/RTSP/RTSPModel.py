...
from RTSPModel_LIB import AddAndInstanceNormalization, FeedForward, MixedScore_MultiHeadAttention

class WeightedMergeBounds(nn.Module):
    def __init__(self):
        super(WeightedMergeBounds, self).__init__()
        self.weight = nn.Parameter(torch.tensor(1.0))

    def forward(self, upper_bound, lower_bound):
        merged_bound = (self.weight * upper_bound) + ((1 - self.weight) * lower_bound)
        return merged_bound


class RTSPModel(nn.Module):

    def __init__(self, **model_params):
        super().__init__()
        self.model_params = model_params
        self.encoder = RTSP_Encoder(**model_params)
        self.decoder = RTSP_Decoder(**model_params)
        self.encoded_row = None
        self.encoded_col = None
        # shape: (batch, node, embedding)
        # Weighted_Merge_dis
        self.merge_dis_model = WeightedMergeBounds()


    def pre_forward(self, reset_state_up, reset_state_down):

        dis_up = reset_state_up.problems
        dis_down = reset_state_down.problems
        # problems.shape: (batch, node, node)

        batch_size = dis_up.size(0)
        node_cnt = dis_down.size(1)
        embedding_dim = self.model_params['embedding_dim']

        #### Initialize the node representation (because the vector representation is an ordered list, and the edges of the graph do not provide a specific order, it is not appropriate to directly use the edge weights of the graph)
        ## row uses a zero vector to embed --> to support variable-sized input
        row_emb = torch.zeros(size=(batch_size, node_cnt, embedding_dim))
        # emb.shape: (batch, node, embedding)

        ## col uses one-hot vector embed or random vector (but the performance of random vector will be weakened a little)
        ...
        # blended
        # dis_avg = self.merge_dis_model(dis_up, dis_down)
        # self.encoded_row, self.encoded_col = self.encoder(row_emb, col_emb, dis_avg)
        # Encoder that weights the upper and lower bound matrices respectively
        self.encoded_row_up, self.encoded_col_up = self.encoder(row_emb, col_emb, dis_up)
        self.encoded_row_down, self.encoded_col_down = self.encoder(row_emb, col_emb, dis_down)
        self.encoded_row = self.encoded_row_up + self.encoded_row_down
        self.encoded_col = self.encoded_col_up + self.encoded_col_down
